...
int runSelfTests(const QString &outputDirectory){
    try{
        ...
        require(savedRayStep&&savedRayGap&&savedStartOpacity&&savedEndOpacity&&savedFadeLength&&resetSavedDefaults&&savedRayStep->value()==7.5&&savedRayGap->value()==20&&savedStartOpacity->value()==15&&savedEndOpacity->value()==80&&savedFadeLength->value()==60,"saved common perspective settings were not applied to a new document");
        const int beforeReset=defaultsWindow.canvas()->undoStack()->count();resetSavedDefaults->click();require(defaultsWindow.canvas()->state().rayStepDegrees==10&&defaultsWindow.canvas()->state().rayGap==12&&defaultsWindow.canvas()->state().rayStartOpacity==10&&defaultsWindow.canvas()->state().rayEndOpacity==70&&defaultsWindow.canvas()->state().rayFadeLength==50&&defaultsWindow.canvas()->undoStack()->count()==beforeReset+1&&!QSettings().contains("perspective/common/rayStepDegrees"),"reset common perspective settings did not restore factory values in one operation");defaultsWindow.canvas()->undoStack()->setClean();defaultsWindow.close();
        ...
        DrawingState loaded;require(Project::load(projectPath,&loaded,&error),qPrintable(error));require(loaded.image==canvas->state().image&&loaded.vanishing==canvas->state().vanishing&&loaded.horizonY==canvas->state().horizonY&&loaded.gridVisible,"DRW roundtrip mismatch");
        ...
        require(loaded.vanishing.y()==210&&loaded.horizonY==240&&loaded.rayStepDegrees==7.5&&loaded.rayGap==20&&loaded.rayStartOpacity==15&&loaded.rayEndOpacity==80&&loaded.rayFadeLength==60,"perspective ray settings did not roundtrip");
