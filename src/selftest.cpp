...
int runSelfTests(const QString &outputDirectory){
    try{
        ...
        auto *vanishingPointSettings=window.findChild<QGroupBox*>("vanishingPointSettings");auto *vanishingPointsList=window.findChild<QGroupBox*>("vanishingPointsList");auto *vanishingPoint1Settings=window.findChild<QGroupBox*>("vanishingPoint1Settings");auto *gridColor=window.findChild<QPushButton*>("gridColor");auto *savePerspectiveDefaults=window.findChild<QPushButton*>("savePerspectiveDefaults");auto *resetPerspectiveDefaults=window.findChild<QPushButton*>("resetPerspectiveDefaults");
        require(vanishingPointSettings&&vanishingPointSettings->title()==QStringLiteral("Настройки точек схода")&&vanishingPointsList&&vanishingPoint1Settings&&gridColor&&savePerspectiveDefaults&&resetPerspectiveDefaults&&vanishingPointSettings->isAncestorOf(rayStep)&&vanishingPointSettings->isAncestorOf(rayGap)&&vanishingPointSettings->isAncestorOf(rayStartOpacity)&&vanishingPointSettings->isAncestorOf(rayEndOpacity)&&vanishingPointSettings->isAncestorOf(rayFadeLength)&&vanishingPointSettings->isAncestorOf(savePerspectiveDefaults)&&vanishingPointSettings->isAncestorOf(resetPerspectiveDefaults)&&vanishingPointsList->isAncestorOf(vanishingPoint1Settings)&&vanishingPoint1Settings->isAncestorOf(gridColor),"perspective panel must separate point settings from the point list");
        ...
        DrawingState styled=initial;styled.gridVisible=true;styled.vanishing.setY(210);styled.rayStepDegrees=7.5;styled.rayGap=20;styled.rayStartOpacity=15;styled.rayEndOpacity=80;styled.rayFadeLength=60;styled.horizonColor=QColor("#d43b3b");styled.horizonOpacity=100;styled.horizonWidth=3;
        ...
        require(loaded.vanishing.y()==210&&loaded.horizonY==240&&!loaded.gridVisible&&loaded.gridColor==QColor("#628ed1")&&loaded.rayStepDegrees==10&&loaded.rayGap==12&&loaded.rayStartOpacity==10&&loaded.rayEndOpacity==70&&loaded.rayFadeLength==50&&loaded.horizonColor==QColor("#628ed1")&&loaded.horizonOpacity==70&&loaded.horizonWidth==1,"DRW must restore perspective geometry without stored appearance");
