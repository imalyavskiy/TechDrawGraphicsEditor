...
int runSelfTests(const QString &outputDirectory) {
    try {
        ...
        require(newMenuAction && openMenuAction && saveMenuAction && undoMenuAction && redoMenuAction &&
                    widthMenuAction && frontMenuAction && backMenuAction && swapMenuAction &&
                    fileMenu->actions().contains(newMenuAction) && fileMenu->actions().contains(openMenuAction) &&
                    fileMenu->actions().contains(saveMenuAction) && editMenu->actions().contains(undoMenuAction) &&
                    editMenu->actions().contains(redoMenuAction) && toolsMenu->actions().contains(widthMenuAction) &&
        ...
        require(vanishingPointSettings &&
                    ...
                    addPoint->text() == QStringLiteral("Добавить") && removePoint && gridColor &&
                    savePerspectiveDefaults && resetPerspectiveDefaults &&
                    vanishingPointSettings->isAncestorOf(rayStep) && vanishingPointSettings->isAncestorOf(rayGap) &&
                    vanishingPointSettings->isAncestorOf(rayStartOpacity) &&
        ...
        require(perspectiveLayout &&
                    ...
                    perspectiveLayout->indexOf(vanishingPointSettings) <
                        perspectiveLayout->indexOf(vanishingPointsList),
        ...
        require(perspectiveDock && perspectiveDock->windowTitle() == QStringLiteral("Перспектива") && horizonToggle &&
                    ...
                    commonFrame->frameShape() == QFrame::StyledPanel &&
                    pointsFrame->frameShape() == QFrame::StyledPanel &&
        ...
        require(pointX && pointY && pointUnits && horizonPosition && horizonUnits && verticalPosition &&
                    verticalUnits && pointAttachment && pointAttachment->count() == 4 && canvasPosition &&
                    pointUnits->currentIndex() == 0 && horizonUnits->currentIndex() == 0 &&
                    verticalUnits->currentIndex() == 0 && verticalPosition->value() == 0 &&
                    std::abs(pointX->value() - 15.0) < 0.01 && std::abs(pointY->value() - 11.29) < 0.02,
        ...
        rayGap->setValue(21);
        require(savePerspectiveDefaults->isEnabled(),
                "changing one saved perspective value must enable the save button");
        rayGap->setValue(20);
        require(!savePerspectiveDefaults->isEnabled(),
                "restoring saved perspective values must disable the save button");
        ...
        require(savedRayStep && savedRayGap && savedStartOpacity && savedEndOpacity && savedFadeLength &&
                    savedPattern && saveSavedDefaults && resetSavedDefaults && savedRayStep->value() == 7.5 &&
                    savedRayGap->value() == 20 && savedStartOpacity->value() == 15 && savedEndOpacity->value() == 80 &&
                    savedFadeLength->value() == 60 && savedPattern->currentIndex() == 1 &&
                    !saveSavedDefaults->isEnabled(),
        ...
        require(defaultsWindow.canvas()->state().rayStepDegrees == 10 &&
                    defaultsWindow.canvas()->state().rayGap == 12 &&
        ...
        require(canvas->state().vanishingPoints[1].position == QPointF(280, 240),
                "horizontal symmetry movement failed");
        ...
        require(!fourPointCanvas.state().vanishingPoints[2].visible &&
                    fourPointCanvas.state().vanishingPoints[0].visible &&
        ...
        require(cursorCanvas.cursor().shape() == Qt::OpenHandCursor,
                "hovering an unlocked point must show an open hand");
        ...
        require(cursorCanvas.cursor().shape() == Qt::OpenHandCursor,
                "hovering the main vertical must show an open hand");
        ...
        require(canvas->state().image.pixelColor(350, 300) == QColor("#26364a"),
                "ctrl shift must constrain segment angle");
        canvas->setDocument(initial);
        ...
        require(canvas->state().image.pixelColor(160, 180) == QColor("#e8cf9b"),
                "eraser straight segment must use Back");
        ...
        require(QLineF(canvas->toImage(canvas->toView(point)), point).length() < 0.001,
                "view coordinate roundtrip failed");
        ...
        require(loadedHistory.states.size() == savedHistory.states.size() &&
                    loadedHistory.labels == savedHistory.labels && loadedHistory.index == savedHistory.index,
        ...
        require(restored.state().image == restoredCurrent.image &&
                    restored.state().vanishingPoints == restoredCurrent.vanishingPoints &&
                    restored.undoStack()->isClean(),
        ...
        require(loaded.vanishingPoints[0].name == QStringLiteral("Главная точка") &&
                    ...
                    loaded.rayFadeLength == 50 && loaded.horizonColor == QColor("#628ed1") &&
                    loaded.horizonOpacity == 70 && loaded.horizonWidth == 1 &&
                    loaded.verticalColor == QColor("#9b6bc0") && loaded.verticalOpacity == 70 &&
        ...
        require(styledView.pixelColor(qRound(styledVanishing.x()), qRound(styledPaper.bottom() + 12)) ==
                    QColor("#dce0e5"),
        ...
        require(
            hasGuidePixel(outsideVanishing + QPointF(52, 30)) && !hasGuidePixel(outsideVanishing + QPointF(0, 50)) &&
        ...
        require(pathWindow.canvas()->state().vanishingPoints == canvas->state().vanishingPoints &&
                    ...
                    pathWindow.canvas()->state().rayEndOpacity == 76 &&
                    pathWindow.canvas()->state().rayFadeLength == 56,
        ...
        require(!Project::save(out.filePath("missing/fail.drw"), loaded, &error),
                "save to missing directory should fail");
        QZipWriter invalid(out.filePath("version.drw"));
        invalid.addFile("drawing.png", QByteArray("bad"));
        invalid.addFile("project.json",
                        QByteArray("{\"format\":\"Drawing\",\"version\":99,\"image\":\"drawing.png\"}"));
        invalid.close();
        ...
        require(Project::load(out.filePath("legacy-v1.drw"), &legacyHistory, &error) &&
                    legacyHistory.states.size() == 1 && legacyHistory.index == 0 && legacyHistory.labels.isEmpty() &&
        ...
        if (QGuiApplication::platformName() != "offscreen") {
            ...
            require(!window.close() && cancelClicked && window.isVisible(),
                    "cancel must prevent closing dirty document");
            canvas->undoStack()->setClean();
