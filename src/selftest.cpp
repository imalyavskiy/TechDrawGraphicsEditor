...
int runSelfTests(const QString &outputDirectory){
    try{
        ...
        const int beforeReset=defaultsWindow.canvas()->undoStack()->count();resetSavedDefaults->click();require(defaultsWindow.canvas()->state().rayStepDegrees==10&&defaultsWindow.canvas()->state().rayGap==12&&defaultsWindow.canvas()->state().rayStartOpacity==10&&defaultsWindow.canvas()->state().rayEndOpacity==70&&defaultsWindow.canvas()->state().rayFadeLength==50&&defaultsWindow.canvas()->undoStack()->count()==beforeReset+1&&!QSettings().contains("perspective/common/rayStepDegrees")&&!saveSavedDefaults->isEnabled(),"reset common perspective settings did not restore factory values in one operation");defaultsWindow.canvas()->undoStack()->setClean();defaultsWindow.close();
