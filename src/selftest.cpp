...
int runSelfTests(const QString &outputDirectory){
    try{
        ...
        DrawingState outside=styled;outside.vanishing=QPointF(-50,100);outside.horizonY=100;outside.rayStepDegrees=90;outside.rayGap=0;outside.rayStartOpacity=100;outside.rayEndOpacity=100;outside.rayFadeLength=0;styledCanvas.setDocument(outside);
        ...
        require(filteredView.pixelColor(qRound(outsideVanishing.x()+30),qRound(outsideVanishing.y()))!=QColor("#dce0e5")&&filteredView.pixelColor(qRound(outsideVanishing.x()),qRound(outsideVanishing.y()+50))==QColor("#dce0e5"),"perspective rays that miss the canvas must be hidden");
