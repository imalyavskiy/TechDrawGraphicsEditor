...
void applySavedViewSettings(DrawingState *state){QSettings settings;state->rayWidth=qBound(0.1,settings.value("perspective/view/rayWidth",1.0).toDouble(),20.0);state->rayAngleOffset=qBound(-180.0,settings.value("perspective/view/rayAngleOffset",0.0).toDouble(),180.0);state->horizonVisible=settings.value("perspective/view/horizonVisible",true).toBool();state->verticalVisible=settings.value("perspective/view/verticalVisible",true).toBool();state->axesVisible=settings.value("perspective/view/axesVisible",false).toBool();state->markersVisible=settings.value("perspective/view/markersVisible",true).toBool();const bool legacySymmetry=settings.value("perspective/view/symmetricPoints",false).toBool();state->horizonSymmetry=settings.value("perspective/view/horizonSymmetry",legacySymmetry).toBool();state->verticalSymmetry=settings.value("perspective/view/verticalSymmetry",legacySymmetry).toBool();const QColor horizonColor(settings.value("perspective/view/horizonColor",QStringLiteral("#628ed1")).toString());state->horizonColor=horizonColor.isValid()?horizonColor:QColor("#628ed1");state->horizonOpacity=qBound(0,settings.value("perspective/view/horizonOpacity",70).toInt(),100);state->horizonWidth=qBound(0.1,settings.value("perspective/view/horizonWidth",1.0).toDouble(),20.0);const QColor verticalColor(settings.value("perspective/view/verticalColor",QStringLiteral("#9b6bc0")).toString());state->verticalColor=verticalColor.isValid()?verticalColor:QColor("#9b6bc0");state->verticalOpacity=qBound(0,settings.value("perspective/view/verticalOpacity",70).toInt(),100);state->verticalWidth=qBound(0.1,settings.value("perspective/view/verticalWidth",1.0).toDouble(),20.0);}
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), canvas_(new Canvas(this)) {
    ...
    verticalPosition_=new QDoubleSpinBox;verticalPosition_->setObjectName("verticalPosition");verticalPosition_->setRange(-1000000,1000000);verticalPosition_->setDecimals(2);verticalPosition_->setKeyboardTracking(false);verticalForm->addRow(tr("Положение X"),verticalPosition_);
    verticalUnits_=new QComboBox;verticalUnits_->setObjectName("verticalUnits");verticalUnits_->addItems({tr("Проценты"),tr("Пиксели")});verticalForm->addRow(tr("Единицы"),verticalUnits_);
    ...
    pointAttachment_=new QComboBox;pointAttachment_->setObjectName("vanishingPointAttachment");pointAttachment_->addItems({tr("Свободна"),tr("Прикреплена к горизонту"),tr("Прикреплена к главной вертикали"),tr("Прикреплена к пересечению осей")});pointForm->addRow(tr("Состояние"),pointAttachment_);
    ...
    connect(horizonSymmetry_,&QCheckBox::toggled,this,[this](bool value){QSettings().setValue("perspective/view/horizonSymmetry",value);canvas_->setHorizonSymmetry(value);});
    ...
    connect(verticalColorButton_,&QPushButton::clicked,this,[this]{auto c=QColorDialog::getColor(canvas_->state().verticalColor,this,tr("Цвет главной вертикали"));if(c.isValid()){QSettings().setValue("perspective/view/verticalColor",c.name(QColor::HexArgb));canvas_->setVerticalColor(c);}});
    connect(verticalOpacity_,qOverload<int>(&QSpinBox::valueChanged),this,[this](int value){QSettings().setValue("perspective/view/verticalOpacity",value);canvas_->setVerticalOpacity(value);});
    connect(verticalWidth_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[this](double value){QSettings().setValue("perspective/view/verticalWidth",value);canvas_->setVerticalWidth(value);});
    connect(verticalVisible_,&QCheckBox::toggled,this,[this](bool value){QSettings().setValue("perspective/view/verticalVisible",value);canvas_->setVerticalVisible(value);});
    connect(verticalLocked_,&QCheckBox::toggled,canvas_,&Canvas::setVerticalLocked);
    connect(verticalSymmetry_,&QCheckBox::toggled,this,[this](bool value){QSettings().setValue("perspective/view/verticalSymmetry",value);canvas_->setVerticalSymmetry(value);});
    connect(verticalPosition_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[this](double value){canvas_->setVerticalX(imageX(value));});
    connect(verticalUnits_,qOverload<int>(&QComboBox::currentIndexChanged),this,[this](int index){setCoordinateUnits(index==0);});
    ...
    connect(pointAttachment_,qOverload<int>(&QComboBox::currentIndexChanged),this,[this](int index){canvas_->setSelectedPointAttachment(index==1?QStringLiteral("horizon"):index==2?QStringLiteral("vertical"):index==3?QStringLiteral("intersection"):QString());});
...
void MainWindow::updateState(){
    ...
    QSignalBlocker r(rayAngleOffset_),s(rayPattern_),t(rayWidth_),u(horizonVisible_),v(axesVisible_),w(markersVisible_),x(horizonSymmetry_),y(horizonLocked_),z(selectedPointLocked_);
    QSignalBlocker aa(verticalWidth_),ab(verticalPosition_),ac(verticalOpacity_),ad(verticalVisible_),ae(verticalLocked_),af(verticalUnits_),ag(verticalSymmetry_);
    gridVisible_->setChecked(canvas_->state().gridVisible);axesVisible_->setChecked(canvas_->state().axesVisible);markersVisible_->setChecked(canvas_->state().markersVisible);rayStep_->setValue(canvas_->state().rayStepDegrees);rayAngleOffset_->setValue(canvas_->state().rayAngleOffset);rayPattern_->setCurrentIndex(canvas_->state().rayPattern);rayWidth_->setValue(canvas_->state().rayWidth);rayGap_->setValue(canvas_->state().rayGap);
    ...
    horizonOpacity_->setValue(canvas_->state().horizonOpacity);horizonWidth_->setValue(canvas_->state().horizonWidth);horizonVisible_->setChecked(canvas_->state().horizonVisible);horizonLocked_->setChecked(canvas_->state().horizonLocked);horizonSymmetry_->setChecked(canvas_->state().horizonSymmetry);horizonPosition_->setSuffix(coordinatePercent_?" %":tr(" px"));horizonPosition_->setValue(displayedY(canvas_->state().horizonY));horizonUnits_->setCurrentIndex(coordinatePercent_?0:1);
    verticalOpacity_->setValue(canvas_->state().verticalOpacity);verticalWidth_->setValue(canvas_->state().verticalWidth);verticalVisible_->setChecked(canvas_->state().verticalVisible);verticalLocked_->setChecked(canvas_->state().verticalLocked);verticalSymmetry_->setChecked(canvas_->state().verticalSymmetry);verticalPosition_->setSuffix(coordinatePercent_?" %":tr(" px"));verticalPosition_->setValue(displayedX(canvas_->state().verticalX));verticalUnits_->setCurrentIndex(coordinatePercent_?0:1);
    ...
    if(hasPoint){pointX_->setValue(displayedX(points[selected].position.x()));pointY_->setValue(displayedY(points[selected].position.y()));const bool onHorizon=points[selected].isAttachedTo(QStringLiteral("horizon")),onVertical=points[selected].isAttachedTo(QStringLiteral("vertical"));pointX_->setEnabled(!onVertical);pointY_->setEnabled(!onHorizon);pointAttachment_->setCurrentIndex(onHorizon&&onVertical?3:onHorizon?1:onVertical?2:0);selectedPointLocked_->setChecked(points[selected].locked);}
    else{pointX_->setValue(0);pointY_->setValue(0);pointAttachment_->setCurrentIndex(0);selectedPointLocked_->setChecked(false);}
    selectedPointVisible_->setChecked(hasPoint&&points[selected].visible);colorSwatch(gridColorButton_,hasPoint?points[selected].color:QColor("#d0d0d0"));colorSwatch(horizonColorButton_,canvas_->state().horizonColor);colorSwatch(verticalColorButton_,canvas_->state().verticalColor);
}
