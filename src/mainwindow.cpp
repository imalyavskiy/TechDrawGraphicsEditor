...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), canvas_(new Canvas(this)) {
    ...
    symmetricPoints_=new QCheckBox(tr("Симметрия связанных пар"));symmetricPoints_->setObjectName("symmetricPoints");symmetricPoints_->setToolTip(tr("Левая и правая точки отражаются относительно главной вертикали, верхняя и нижняя — относительно горизонта"));commonForm->addRow(symmetricPoints_);
    ...
    auto *pointButtons=new QHBoxLayout;auto *addPointButton=new QPushButton(tr("На горизонте"));addPointButton->setObjectName("addVanishingPoint");auto *addVerticalPointButton=new QPushButton(tr("На вертикали"));addVerticalPointButton->setObjectName("addVerticalVanishingPoint");removePointButton_=new QPushButton(tr("Удалить"));removePointButton_->setObjectName("removeVanishingPoint");pointButtons->addWidget(addPointButton);pointButtons->addWidget(addVerticalPointButton);pointButtons->addWidget(removePointButton_);pointsListLayout->addLayout(pointButtons);
    ...
    connect(addVerticalPointButton,&QPushButton::clicked,this,[this]{canvas_->addVerticalVanishingPoint();const int i=canvas_->selectedPointIndex();if(i<0)return;QSettings settings;const QColor color(settings.value(QString("perspective/points/%1/color").arg(i),defaultPointColor(i)).toString());canvas_->setSelectedPointColor(color.isValid()?color:defaultPointColor(i));canvas_->setSelectedPointVisible(settings.value(QString("perspective/points/%1/visible").arg(i),true).toBool());});
...
void MainWindow::updateState(){
    ...
    QSignalBlocker aa(verticalWidth_),ab(verticalPosition_),ac(verticalOpacity_),ad(verticalVisible_),ae(verticalLocked_),af(verticalUnits_);
