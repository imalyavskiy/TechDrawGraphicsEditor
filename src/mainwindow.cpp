...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), canvas_(new Canvas(this)) {
    ...
    pointAttachment_=new QComboBox;pointAttachment_->setObjectName("vanishingPointAttachment");pointAttachment_->addItems({tr("Свободна"),tr("Прикреплена к горизонту"),tr("Прикреплена к главной вертикали")});pointForm->addRow(tr("Состояние"),pointAttachment_);
    ...
    connect(pointAttachment_,qOverload<int>(&QComboBox::currentIndexChanged),this,[this](int index){canvas_->setSelectedPointAttachment(index==1?QStringLiteral("horizon"):index==2?QStringLiteral("vertical"):QString());});
