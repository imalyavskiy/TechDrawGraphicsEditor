    ...
    if(formatVersion>=4){
        ...
        for(int pointIndex=0;pointIndex<points.size();++pointIndex){
            ...
            if(!attachment.isNull()&&!attachment.isUndefined()){
                ...
                point.attachmentType=binding.value("type").toString();point.attachmentTargetId=binding.value("targetId").toString();
                const bool knownTarget=point.attachmentTargetId==QStringLiteral("horizon")||(formatVersion>=6&&point.attachmentTargetId==QStringLiteral("vertical"));
                if(point.attachmentType!=QStringLiteral("construction")||!knownTarget)return fail(error,QStringLiteral("Неизвестная цель привязки точки схода."));
            }
            parsed.append(point);
        }
