...
QJsonObject perspectiveJson(const DrawingState &state) {
    ...
    for(const auto &point:state.vanishingPoints){
        QJsonArray attachments;for(const auto &targetId:point.attachmentTargetIds)attachments.append(QJsonObject{{"type","construction"},{"targetId",targetId}});
        points.append(QJsonObject{{"id",point.id},{"name",point.name},{"x",point.position.x()},{"y",point.position.y()},{"locked",point.locked},{"attachments",attachments}});
    }
...
bool parsePerspective(const QJsonValue &value,DrawingState *state,QString *error,int formatVersion) {
    ...
    if(formatVersion>=4){
        ...
        double verticalX=state->image.width()/2.0;bool verticalLocked=false;
        if(formatVersion>=6){
            if(!perspective.value("vertical").isObject())return fail(error,QStringLiteral("Отсутствует главная вертикаль."));
            const auto vertical=perspective.value("vertical").toObject();verticalX=vertical.value("x").toDouble(qQNaN());
            if(!std::isfinite(verticalX)||std::abs(verticalX)>1000000)return fail(error,QStringLiteral("Положение главной вертикали вне допустимого диапазона."));
            if(!vertical.value("locked").isBool())return fail(error,QStringLiteral("Некорректное состояние фиксации главной вертикали."));
        ...
        for(int pointIndex=0;pointIndex<points.size();++pointIndex){
            ...
            ids.insert(point.id);
            if(formatVersion>=8){
                const auto attachments=object.value("attachments");if(!attachments.isArray())return fail(error,QStringLiteral("Некорректные привязки точки схода."));
                QSet<QString> targets;for(const auto &entry:attachments.toArray()){
                    if(!entry.isObject())return fail(error,QStringLiteral("Некорректная привязка точки схода."));
                    const auto binding=entry.toObject();const QString type=binding.value("type").toString(),targetId=binding.value("targetId").toString();
                    if(type!=QStringLiteral("construction")||(targetId!=QStringLiteral("horizon")&&targetId!=QStringLiteral("vertical"))||targets.contains(targetId))return fail(error,QStringLiteral("Неизвестная или повторная цель привязки точки схода."));
                    targets.insert(targetId);point.attachmentTargetIds.append(targetId);
                }
            }else{
                ...
                if(!attachment.isNull()&&!attachment.isUndefined()){
                    ...
                    const auto binding=attachment.toObject();const QString type=binding.value("type").toString(),targetId=binding.value("targetId").toString();
                    const bool knownTarget=targetId==QStringLiteral("horizon")||(formatVersion>=6&&targetId==QStringLiteral("vertical"));if(type!=QStringLiteral("construction")||!knownTarget)return fail(error,QStringLiteral("Неизвестная цель привязки точки схода."));point.attachmentTargetIds.append(targetId);
                }
            }
            parsed.append(point);
        }
        state->horizonY=horizonY;state->horizonLocked=formatVersion>=5&&horizon.value("locked").toBool();state->verticalX=verticalX;state->verticalLocked=verticalLocked;state->vanishingPoints=parsed;return true;
    }
    ...
    if(qFuzzyCompare(y+1,horizonY+1))point.attachmentTargetIds.append(QStringLiteral("horizon"));
...
bool validState(const DrawingState &state) {
    ...
    QSet<QString> ids;for(const auto &point:state.vanishingPoints){if(point.id.isEmpty()||point.id.size()>80||point.name.size()>120||ids.contains(point.id)||!std::isfinite(point.position.x())||!std::isfinite(point.position.y())||std::abs(point.position.x())>1000000||std::abs(point.position.y())>1000000||!point.color.isValid()||point.attachmentTargetIds.size()>2)return false;ids.insert(point.id);QSet<QString> targets;for(const auto &targetId:point.attachmentTargetIds){if((targetId!=QStringLiteral("horizon")&&targetId!=QStringLiteral("vertical"))||targets.contains(targetId))return false;targets.insert(targetId);}}
    return
        ...
        std::isfinite(state.horizonWidth)&&state.horizonWidth>=0.1&&state.horizonWidth<=20&&state.verticalColor.isValid()&&state.verticalOpacity>=0&&state.verticalOpacity<=100&&std::isfinite(state.verticalWidth)&&state.verticalWidth>=0.1&&state.verticalWidth<=20;
}
bool samePersistentState(const DrawingState &a,const DrawingState &b) {
    ...
    for(int i=0;i<a.vanishingPoints.size();++i){const auto &x=a.vanishingPoints[i],&y=b.vanishingPoints[i];if(x.id!=y.id||x.name!=y.name||x.position!=y.position||x.attachmentTargetIds!=y.attachmentTargetIds||x.locked!=y.locked)return false;}return true;
}
}
...
bool Project::save(const QString &path,const DrawingHistory &history,QString *error) {
    ...
    QJsonObject metadata{{"format","Drawing"},{"version",8},{"width",size.width()},{"height",size.height()},
...
bool Project::load(const QString &path,DrawingHistory *history,QString *error) {
    ...
    if (object.value("format").toString()!="Drawing"||(versionValue!=1&&versionValue!=2&&versionValue!=3&&versionValue!=4&&versionValue!=5&&versionValue!=6&&versionValue!=7&&versionValue!=8)||object.value("image").toString()!="drawing.png")
