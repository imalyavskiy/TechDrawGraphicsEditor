...
Модель документа уже рассчитана на упорядоченный стек разных типов слоёв. На текущем переходном шаге пользовательские команды создают один растровый слой; операции с несколькими слоями и их панель добавляются в этапе 8. Камеры, трёхмерной сцены и физических параметров объектива в модели нет. Перспектива задаётся только геометрическими объектами на плоскости холста.
...
`ToolPropertiesPanel` находится внутри левой панели инструментов под кнопками выбора. Часто используемые параметры и дополнительные свойства наконечника разнесены по двум сворачиваемым секциям. Панель получает только модель настроек и `DrawingTargetContext`; она не обращается к растровым данным. Пока контекст сохраняет прежнее однослойное поведение без доступной прозрачности; следующий шаг этапа 8 будет формировать его из активной записи `LayerStack`. Поэтому ластик пока сообщает «Цветом Back», но интерфейс уже умеет показать «До прозрачности» или причину недоступности без замены панели.
...
`project.cpp` — единственная подсистема, знающая устройство `.drw`. Она проверяет размеры и ограничения, преобразует геометрию в JSON, кодирует растры проекта в PNG, читает прежние версии и атомарно заменяет файлы через `QSaveFile`. Пока схема остаётся версией 8, `Project` сохраняет сведённую `LayerCompositor` композицию и при чтении создаёт из неё один растровый слой. Полное многослойное хранение вводится отдельным следующим изменением формата. Обычный экспорт поддерживает PNG, JPEG и BMP. Для JPEG и BMP изображение предварительно сводится на белый фон, поскольку эти варианты не сохраняют прозрачность; PNG сохраняет альфа-канал.
...
- `workspace/tools/*`, `workspace/perspective/*` — видимость, закрепление и последняя ширина боковых панелей;
