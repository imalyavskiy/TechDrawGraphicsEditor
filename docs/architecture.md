...
Точка входа задаёт DPI-режим, технические идентификаторы приложения, шрифт, значок и мигрирует прежние настройки `DrawingPrototype/Drawing`. Обычный запуск создаёт `MainWindow`; параметр `--self-test <каталог>` передаёт управление встроенным проверкам без входа в основной цикл редактора.
...
Корневой `CMakeLists.txt` создаёт одну цель `TechDraw`, включает C++17, AUTOMOC/AUTORCC, Qt Widgets и закрытую цель Qt Gui. Для MinGW Windows-ресурс компилируется отдельной командой, чтобы `windres` работал с путями, содержащими пробелы.

Сценарии `scripts/windows` составляют поддерживаемый интерфейс сборки. Release-комплект разворачивается в `dist/TechDraw`; IExpress упаковывает его и PowerShell-установщик в `dist/installer/TechnicalDrawing-Setup.exe`. Подробные команды находятся в [howto.md](howto.md).
