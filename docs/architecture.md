...
Панель находится справа под панелью перспективы и использует собственный `AutoHideDockWidget`. Список отображает слои сверху вниз в порядке, обратном внутреннему порядку композиции. Строка содержит миниатюру зарегистрированного рендерера, тип, редактируемое имя, глаз и замок. Общие команды создают, удаляют, дублируют и переставляют слои; свойства активной записи управляют непрозрачностью, доступностью и фиксацией альфа-канала. Все документные изменения делегируются `Canvas` и становятся командами Undo/Redo.
