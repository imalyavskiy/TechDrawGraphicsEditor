...
Меню `Изображение → Направляющие` создаёт обычные направляющие в точной позиции от верхнего или левого края и делегирует удаление `Canvas`. Позиция вводится в пикселях либо процентах полного соответствующего размера холста. Команда прилипания в меню `Вид` и кнопка главного тулбара используют один `QAction`, поэтому состояние не может разойтись между двумя представлениями.
...
- `view/guides/*` — независимые состояния показа и общего режима прилипания направляющих;
