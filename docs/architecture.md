...
Компонент добавляет операции ярлыков, ассоциации `.drw`, записи удаления и одноразового намерения закрепления на панели задач. QtIFW журналирует извлечение и сопутствующие операции для отката и удаления; `RemoveTargetDir=false` не позволяет деинсталлятору удалить постороннее содержимое выбранного каталога. При замене прежней IExpress-версии ограниченный `migrate-legacy.ps1` читает только её закрытый манифест владения, сохраняет пользовательские документы и удаляет прежние записи после развёртывания нового пакета. Области CurrentUser и AllUsers имеют раздельные каталоги, ярлыки и ветви реестра. Заглушки подписи вызываются после развёртывания бинарных файлов и после создания итогового QtIFW EXE. Подробные команды находятся в [howto.md](howto.md).
