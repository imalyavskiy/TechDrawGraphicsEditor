...
Сценарии `scripts/windows` составляют поддерживаемый интерфейс сборки. Release-комплект вместе с внешним каталогом переводов разворачивается в `dist/TechDraw`; IExpress упаковывает его и PowerShell-установщик в `dist/installer/TechnicalDrawing-Setup.exe`. Подробные команды находятся в [howto.md](howto.md).
