...
Установщик сравнивает версии. Более новая версия обновляется поверх существующей; та же версия предлагает восстановление; более старая требует отдельного подтверждения downgrade. Копия в другой области установки предлагает перенос, сохранение обеих копий или отмену. Qt Installer Framework создаёт единственную видимую запись Windows «Приложения и возможности»; служебные поля TechDraw хранятся отдельно в `HKCU\Software\TechDraw\Installer` или `HKLM\Software\TechDraw\Installer` и не образуют вторую строку приложения. QtIFW регистрирует операции извлечения и отменяет их при ошибке; `<RemoveTargetDir>false>` оставляет постороннее содержимое выбранного каталога, проекты `.drw`, изображения и другие документы. Отдельный ограниченный помощник миграции читает только манифест прежней IExpress-версии и не является новым механизмом установки. Перед удалением старых файлов, ярлыков и записей реестра он создаёт временную резервную копию: QtIFW восстанавливает её при откате установки и удаляет после успеха. Деинсталлятор отдельно предлагает удалить настройки текущего пользователя, причём флажок выключен по умолчанию.
...
Каждый из этих сценариев также создаёт под уникальными временными именами макет прежней IExpress-установки и проверяет применение, откат и завершение миграции. Временные файлы и ключи `HKCU` удаляются в блоке очистки даже при ошибке проверки. Автоматическую запись QtIFW в списке приложений сценарий удаляет только при одновременном точном совпадении `InstallLocation` с изолированным каталогом lifecycle-теста и `UninstallString` с расположенным в нём `TechDrawMaintenance.exe`; совпадения одного имени приложения недостаточно.
