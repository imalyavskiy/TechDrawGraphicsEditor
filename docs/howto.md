...
3. архивирует переносимый комплект в `build\installer-x64` либо `build\installer-x86` и записывает JSON-метаданные с версией, архитектурой и манифестом файлов;
4. формирует директиву IExpress и создаёт `dist\installer\TechnicalDrawing-Setup-x64.exe` либо `TechnicalDrawing-Setup-x86.exe`;
...
Установщик сравнивает версии. Более новая версия обновляется поверх существующей; та же версия предлагает восстановление; более старая требует отдельного подтверждения downgrade. Копия в другой области установки предлагает перенос, сохранение обеих копий или отмену. В частной JSON-регистрации перечисляются принадлежащие приложению файлы: обновление и удаление не затрагивают постороннее содержимое выбранного каталога, проекты `.drw`, изображения и другие документы. Деинсталлятор отдельно предлагает удалить настройки текущего пользователя, причём флажок выключен по умолчанию.
...
```powershell
build\installer-x64\install-techdraw.ps1 `
  -InstallDirectory "$PWD\build\installer-test-install" `
  -SkipShellIntegration -Quiet
```

Полную изолированную проверку чистой установки, repair, архитектуры и безопасного удаления выполняют:
...
- `build\installer-x64`, `build\installer-x86` — промежуточные файлы упаковки;
