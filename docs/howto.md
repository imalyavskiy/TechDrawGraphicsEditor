...
4. Qt 5.15.2 для MinGW x64 с модулями Core, Gui, Widgets, LinguistTools и Svg. Сборка приложения использует закрытые заголовки Qt Gui для `QZipReader` и `QZipWriter`, поэтому комплект разработки должен содержать каталоги версионных закрытых заголовков. В `bin` комплекта должны быть `lupdate.exe` и `lrelease.exe`.
