...
Каждый из этих сценариев также создаёт под уникальными временными именами макет прежней IExpress-установки и проверяет применение, откат и завершение миграции. Временные файлы и ключи `HKCU` удаляются в блоке очистки даже при ошибке проверки.
